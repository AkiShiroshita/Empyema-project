...
kobe <- read_excel("data/kobe.xlsx")
...
request <- kobe %>% 
  ...
  distinct(id, adm)
request %>% write.csv("request/kobe_request.csv")
##
memo <- read_csv("memo/kobe_after.csv",
...
new <- memo %>% 
  pull(5)
num2 <- str_c(num, collapse = ",")
kobe <- kobe %>% 
  select(num2)
memo2 <- str_c(old, collapse = "|")
kobe <- kobe %>% 
  filter(str_detect(, memo2)) 
